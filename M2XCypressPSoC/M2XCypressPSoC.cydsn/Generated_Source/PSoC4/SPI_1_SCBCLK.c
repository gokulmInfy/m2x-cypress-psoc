...
void SPI_1_SCBCLK_SetFractionalDividerRegister(uint16 clkDivider, uint8 clkFractional)
{
    ...
    uint32 maskVal = SPI_1_SCBCLK_DIV_REG & 
                    (uint32)(~(SPI_1_SCBCLK__DIVIDER_MASK | SPI_1_SCBCLK__FRAC_MASK)); 
	/* combine mask and new divider val into 32-bit value */
    uint32 regVal = clkDivider | (((uint32)clkFractional << 16) & SPI_1_SCBCLK__FRAC_MASK) | maskVal;
#else
    /* get all but integer divider bits */
    uint32 maskVal = SPI_1_SCBCLK_DIV_REG & (uint32)(~(uint32)SPI_1_SCBCLK__DIVIDER_MASK);
    /* combine mask and new divider val into 32-bit value */
...
uint8 SPI_1_SCBCLK_GetFractionalDividerRegister(void)
{
    ...
    uint32 maskVal = SPI_1_SCBCLK_DIV_REG & SPI_1_SCBCLK__FRAC_MASK;
    return (maskVal >> 16);
#else
    return 0u;
#endif /* SPI_1_SCBCLK__FRAC_MASK */
